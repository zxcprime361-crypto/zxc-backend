    ...
    >
      ...
      {Array.from({ length: _values.length }, (_, index) => (
        <SliderPrimitive.Thumb
          ...
          key={index}
          className="opacity-0 duration-200  cursor-pointer transition-opacity group-hover:opacity-100 border-primary ring-ring/50 block h-5 w-1.5 shrink-0 rounded-full border bg-white shadow-sm hover:ring-4 focus-visible:ring-4 focus-visible:outline-hidden disabled:pointer-events-none disabled:opacity-50"
        />
      ))}
    </SliderPrimitive.Root>
  );
